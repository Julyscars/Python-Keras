...
def l_in_s(l, s):
    l = [str(i, 'utf-8') if type(i) is not str else i for i in l]
    for i in l:
        if i in s:
            return True
    return False
...
def get_count_dict(string):
    ...
    print ("create count_dict done, costs time: %s" % (str(end - start)))
    return c_dict
...
def get_candidate_dict(string, min_l=2, max_l=5, tf=1, ie=1, pmi=1, exclu_list=['\n', ' ', '：', '。', '“', '”', '！', '？', '，', u'\xa0'] + list(string.punctuation)):
    ...
    print ("create candidate_dict done, costs time: %s" % (str(end - start)))
    start = datetime.datetime.now()
    ...
    print ("computing done, costs time: %s" % (str(end - start)))
    ...
    candidate_list = sorted(candidate_list, key=lambda x: x[1][0] * (x[1][1] + x[1][2]),reverse=True)
    return candidate_list

if __name__ == "__main__":
    data = open('d:/python/newci/123.txt',encoding='utf-8').read()
    string = data
